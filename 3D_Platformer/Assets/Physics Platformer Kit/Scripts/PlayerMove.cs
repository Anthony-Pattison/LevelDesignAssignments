...
public class PlayerMove : MonoBehaviour 
{
	...
	void FixedUpdate() 
	{
		...
		if(animator)
		{
			...
			animator.SetFloat("YVelocity", GetComponent<Rigidbody>().linearVelocity.y);
		}
	}
	...
	void OnCollisionStay(Collision other)
	{
		...
		if(direction.magnitude == 0 && slope < slopeLimit && rigid.linearVelocity.magnitude < 2)
		{
		...
		foreach (Transform check in floorCheckers)
		{
			...
			if(Physics.Raycast(check.position, Vector3.down, out hit, dist + 0.05f))
			{
				if(!hit.transform.GetComponent<Collider>().isTrigger)
				{
					...
					//enemy bouncing
					if (hit.transform.tag == "Enemy" && rigid.linearVelocity.y < 0)
					{
					...
					{
						movingObjSpeed = hit.transform.GetComponent<Rigidbody>().linearVelocity;
						movingObjSpeed.y = 0f;
	...
	private void JumpCalculations()
	{
		...
		//play landing sound
		if(groundedCount < 0.25 && groundedCount != 0 && !GetComponent<AudioSource>().isPlaying && landSound && GetComponent<Rigidbody>().linearVelocity.y < 1)
		{
			aSource.volume = Mathf.Abs(GetComponent<Rigidbody>().linearVelocity.y)/40;
